    ...
    class HuaweiCloud
          ...
          if options['security_groups']
            ...
            data['server']['security_groups'] =
              Array(options['security_groups']).map do |sg|
                name = if sg.kind_of?(Fog::Compute::HuaweiCloud::SecurityGroup)
                         sg.name
                       else
                         sg
                       end
      ...
      class Mock
        def create_server(name, image_ref, flavor_ref, options = {})
          ...
          identity = Fog::Identity::HuaweiCloud.new :huaweicloud_auth_url => credentials[:huaweicloud_auth_url]
          ...
          user_id = if user
                      ...
                      response = identity.create_user(@huaweicloud_username,
                                                      'password',
                                                      "#{@huaweicloud_username}@example.com")
          ...
          if security_groups
            groups = Array(security_groups).map do |sg|
              if sg.kind_of?(Fog::Compute::HuaweiCloud::SecurityGroup)
                sg.name
              else
                sg
              end
